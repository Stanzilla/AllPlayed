...
[*]Install the addon in you Word of Warcraft\Interface\Addons\ directory. There are no dependencies (well, except FuBar for now, see the To Do list).
...
[*]Use Tablet when FuBar is not loaded instead of printing the report in the chat windows
[*]Optional format with more then two columns
[/list]
...
[b]Note:[/b][list]
[*]When the FuBar part of the addon is active, AllPlayed will recompute the data based on the [i]show_sconds[/i] option. If [i]show_seconds[/i] is on, the data will be compute every second. If [i]show_seconds[/i] is off, it will compute every 20 seconds. If you fear that the addon will take mush resource, do not display the seconds. If the addon is disable in FuBar, the automatic refresh is 
