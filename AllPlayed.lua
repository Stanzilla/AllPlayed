...
-- Creation fo the main "object" with libraries (mixins) directly attach to the object (use self:functions)
...
function AllPlayed:Print(message, ...)
    if default_channel then
        _G["ChatFrame" .. default_channel]:AddMessage(
...
function AllPlayed:OnInitialize()
    ...
    -- Register the variables with the defaults
...
function AllPlayed:OnEnable()
    ...
    -- (event unregistering is done automatically by Ace)
    ...
    self.OnUpdate_frame:SetScript(
        ...
        function(self, elap)
            ...
            if AllPlayed.elapsed < AllPlayed:GetOption("tooltip_delay") then
                return
            end
            if
                (AllPlayed.tooltip and AllPlayed.tooltip:IsMouseOver() and
                    AllPlayed:GetOption("tooltip_keep_on_mouseover")) or
                    (AllPlayed.tooltip_anchor and AllPlayed.tooltip_anchor:IsMouseOver())
             then
                AllPlayed.elapsed = 0
                return
            end
...
function AllPlayed:IsDebugging()
...
function AllPlayed:ComputeTotal()
    ...
    for faction, faction_table in pairs(self.db.global.data) do
        for realm, realm_table in pairs(faction_table) do
            ...
            if not self.total_realm[faction] then
                self.total_realm[faction] = {}
            end
            if not self.total_realm[faction][realm] then
                self.total_realm[faction][realm] = {}
            end
            ...
            for pc, pc_table in pairs(realm_table) do
                if not self:GetOption("is_ignored", realm, pc) then
                    ...
                    local seconds_played =
                        self:EstimateTimePlayed(pc, realm, pc_table.seconds_played, pc_table.seconds_played_last_update)
                    ...
                    if pc_table.level == self.max_pc_level then
                        pc_xp = 0
                    end
                    ...
                    self.total_faction[faction].justice_points =
                        self.total_faction[faction].justice_points + pc_table.justice_points
                    self.total_faction[faction].valor_points =
                        self.total_faction[faction].valor_points + pc_table.valor_points
                    self.total_faction[faction].levels = self.total_faction[faction].levels + pc_table.level
                    self.total_realm[faction][realm].time_played =
                        self.total_realm[faction][realm].time_played + seconds_played
                    ...
                    self.total_realm[faction][realm].justice_points =
                        self.total_realm[faction][realm].justice_points + pc_table.justice_points
                    self.total_realm[faction][realm].valor_points =
                        self.total_realm[faction][realm].valor_points + pc_table.valor_points
    ...
    if self:GetOption("all_realms") then
        if self:GetOption("all_factions") then
            ...
            self.total.time_played =
                self.total_faction["Horde"].time_played + self.total_faction["Alliance"].time_played +
                self.total_faction["Neutral"].time_played
            self.total.coin =
                self.total_faction["Horde"].coin + self.total_faction["Alliance"].coin +
                self.total_faction["Neutral"].coin
            self.total.xp =
                self.total_faction["Horde"].xp + self.total_faction["Alliance"].xp + self.total_faction["Neutral"].xp
            self.total.justice_points =
                self.total_faction["Horde"].justice_points + self.total_faction["Alliance"].justice_points +
                self.total_faction["Neutral"].justice_points
            self.total.valor_points =
                self.total_faction["Horde"].valor_points + self.total_faction["Alliance"].valor_points +
                self.total_faction["Neutral"].valor_points
            self.total.levels =
                self.total_faction["Horde"].levels + self.total_faction["Alliance"].levels +
...
function AllPlayed:ComputeTotalHonor()
    ...
    for faction, faction_table in pairs(self.db.global.data) do
        for realm, realm_table in pairs(faction_table) do
            ...
            for pc, pc_table in pairs(realm_table) do
                if not self:GetOption("is_ignored", realm, pc) then
                    self.total_faction[faction].honor_kills =
                        self.total_faction[faction].honor_kills + (pc_table.honor_kills or 0)
                    self.total_faction[faction].honor_points =
                        self.total_faction[faction].honor_points + (pc_table.honor_points or 0)
                    self.total_faction[faction].arena_points =
                        self.total_faction[faction].arena_points + (pc_table.arena_points or 0)
                    self.total_faction[faction].conquest_points =
                        self.total_faction[faction].conquest_points + (pc_table.conquest_points or 0)

                    self.total_realm[faction][realm].honor_kills =
                        self.total_realm[faction][realm].honor_kills + (pc_table.honor_kills or 0)
                    self.total_realm[faction][realm].honor_points =
                        self.total_realm[faction][realm].honor_points + (pc_table.honor_points or 0)
                    self.total_realm[faction][realm].arena_points =
                        self.total_realm[faction][realm].arena_points + (pc_table.arena_points or 0)
                    self.total_realm[faction][realm].conquest_points =
                        self.total_realm[faction][realm].conquest_points + (pc_table.conquest_points or 0)
                end
            end
        end
    end
    ...
    if self:GetOption("all_realms") then
        if self:GetOption("all_factions") then
            ...
            self.total.honor_kills =
                self.total_faction["Horde"].honor_kills + self.total_faction["Alliance"].honor_kills +
                self.total_faction["Neutral"].honor_kills
            self.total.honor_points =
                self.total_faction["Horde"].honor_points + self.total_faction["Alliance"].honor_points +
                self.total_faction["Neutral"].honor_points
            self.total.arena_points =
                self.total_faction["Horde"].arena_points + self.total_faction["Alliance"].arena_points +
                self.total_faction["Neutral"].arena_points
            self.total.conquest_points =
                self.total_faction["Horde"].conquest_points + self.total_faction["Alliance"].conquest_points +
...
-- Fill the QTip with the information
...
function AllPlayed:DrawTooltip(anchor)
    ...
    if
        self:GetOption("show_coins") or self:GetOption("show_rested_xp") or self:GetOption("show_rested_xp_countdown") or
    ...
    for _, faction in ipairs(self.sort_faction) do
        ...
        -- all PC in the faction are ignored.
        if
            ((self:GetOption("all_factions") or self.faction == faction) and self.total.time_played ~= 0 and
                self.sort_faction_realm[self:GetOption("display_sort_type")][faction])
         then
            for _, realm in ipairs(self.sort_faction_realm[self:GetOption("display_sort_type")][faction]) do
                ...
                -- all PC in the realm are ignored.
                if
                    ((self:GetOption("all_realms") or self.realm == realm) and
                        self.total_realm[faction][realm].time_played ~= 0)
                 then
                    ...
                    if self:GetOption("show_lvl_totals") then
                        if first_option then
                            text_realm_optional =
                                C:White((L["Lvls: %d"]):format(self.total_realm[faction][realm].levels))
                            first_option = false
                        else
                            text_realm_optional =
                                ("%s " .. C:Green(" : %s")):format(
                    ...
                    for i, pc in ipairs(self.sort_realm_pc[self:GetOption("display_sort_type")][faction][realm]) do
                        if not self:GetOption("is_ignored", realm, pc) then
                            ...
                            if self:GetOption("show_location") ~= "none" then
                                if
                                    self:GetOption("show_location") == "loc" or pc_data.zone_text == L["Unknown"] or
                                        (self:GetOption("show_location") == "loc/sub" and pc_data.subzone_text == "")
                                 then
                                    col_text[col_no] = FactionColour(faction, pc_data.zone_text)
                                elseif self:GetOption("show_location") == "sub" then
                                    col_text[col_no] = FactionColour(faction, pc_data.subzone_text)
                                else
                                    col_text[col_no] =
                                        FactionColour(faction, pc_data.zone_text .. "/" .. pc_data.subzone_text)
                                end
                            ...
                            if need_last_login then
                                if pc == AllPlayed.pc then
                                    ...
                                    col_text[col_no] =
                                        pc_data.last_update and pc_data.last_update ~= 0 and
                                        (L["|cff00ffffSeen |cffffffff%s |cff00ffffago|r"]):format(
                                            AllPlayed:FormatTime(time() - pc_data.last_update)
                                        ) or
                                        ""
                                end
                                col_align[col_no] = "CENTER"
                            ...
                            if
                                self:GetOption("show_coins") or self:GetOption("show_rested_xp") or
                            ...
                            if (pc_data.level < self.max_pc_level) and (pc_data.level > 1 or pc_data.xp > 0) then
                                -- How much rested XP do we have?
                                local estimated_rested_xp =
                                    self:EstimateRestedXP(
                                    pc,
                                    realm,
                                    pc_data.level,
                                    pc_data.rested_xp,
                                ...
                                if self:GetOption("show_rested_xp") then
                                    ...
                                    col_text[col_no] =
                                        col_text[col_no] ..
                                        FactionColour(faction, L["%d rested XP"]):format(estimated_rested_xp)
                                end
                                ...
                                if pc_data.level == self.max_pc_level - 1 then
                                    ...
                                    rested_percent =
                                        floor(
                                        estimated_rested_xp / XPToNextLevelCache[self.max_pc_level - 1] * 100 + 0.5
                                    ) / 100
                                end
                                ...
                                if percent_for_colour < 1 and
                                        (pc_data.is_resting or pc ~= self.pc or realm ~= self.realm)
                                 then
                                ...
                                if
                                    percent_rest ~= "0" and self:GetOption("show_rested_xp_countdown") and
                                        text_countdown ~= ""
                                 then
                                    col_text[col_no] =
                                        col_text[col_no] ..
                                        PercentColour(percent_for_colour, " (%d%% %s, -%s)"):format(
                                ...
                                elseif self:GetOption("percent_rest") ~= "0" then
                                    col_text[col_no] =
                                        col_text[col_no] ..
                                        PercentColour(percent_for_colour, " (%d%% %s)"):format(
                                            percent_rest * rested_percent,
                                            L["rested"]
                                        )
                                elseif self:GetOption("show_rested_xp_countdown") and text_countdown ~= "" then
                                    col_text[col_no] =
                                        col_text[col_no] ..
                                        PercentColour(percent_for_colour, " (-" .. text_countdown .. ")")
                                end
                            end
                            ...
                            if color then
                                tooltip:SetLineColor(line, color, color, color, self:GetOption("opacity"))
                            end
    ...
    if
        self:GetOption("show_played_time") or self:GetOption("show_coins") or self:GetOption("show_lvl_totals") or
...
-- Function triggered when the TIME_PLAYED_MSG event is fired
...
end -- Options that are be deprecated
--

--[[ Methods used for the option menu ]] local deprecated_options = {
...
function AllPlayed:SetOption(option, value, ...)
    ...
    elseif option == "show_seconds" then
        ...
        if value then
            ...
            -- If only the minutes are shown, 3 refreshes a minute will do nicely
    ...
    elseif option == "opacity" then
        -- Adjust the sort type with the direction
...
function AllPlayed:EstimateRestedXP(pc, realm, level, rested_xp, max_rested_xp, last_update, is_resting, curent_xp)
    ...
    if is_resting then
        ...
        estimated_rested_xp =
            min(rested_xp + floor((time() - last_update) * (max_rested_xp / (4 * TEN_DAYS))), max_rested_xp)
    end
...
-- Function that format the money string
...
function FormatCharacterName(pc, level, xp, seconds_played, class, class_loc, faction, last_update)
    ...
    if AllPlayed:GetOption("show_xp_total") and xp ~= -1 then
        ...
        if AllPlayed.max_pc_level < level then
            pc_xp = pc_xp + xp
        end
...
function AllPlayed:BuildSortTables()
    ...
    if self.sort_tables_done then
        return
    end
    self:Debug("Sorting done: %d", time())

    -- Static sort for the factions
    if not self.sort_faction then
        self.sort_faction = {"Horde", "Alliance", "Neutral"}
    end
    ...
    for faction, faction_table in pairs(self.db.global.data) do
        ...
        self.sort_faction_realm["alpha"][faction] = buildSortedTable(faction_table)
        self.sort_faction_realm["rev-alpha"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["level"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-level"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["xp"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-xp"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rested_xp"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-rested_xp"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["percent_rest"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-percent_rest"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["coin"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-coin"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["time_played"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-time_played"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["ilevel"][faction] = self.sort_faction_realm["alpha"][faction]
        self.sort_faction_realm["rev-ilevel"][faction] = self.sort_faction_realm["alpha"][faction]
        ...
        for realm, realm_table in pairs(faction_table) do
            ...
            self.sort_realm_pc["level"][faction][realm] = buildSortedTable(realm_table, PCSortByLevel)
            self.sort_realm_pc["rev-level"][faction][realm] = buildSortedTable(realm_table, PCSortByRevLevel)
            self.sort_realm_pc["xp"][faction][realm] = buildSortedTable(realm_table, PCSortByXP)
            self.sort_realm_pc["rev-xp"][faction][realm] = buildSortedTable(realm_table, PCSortByRevXP)
            self.sort_realm_pc["rested_xp"][faction][realm] = buildSortedTable(realm_table, PCSortByRestedXP, realm)
            ...
            self.sort_realm_pc["coin"][faction][realm] = buildSortedTable(realm_table, PCSortByCoin)
            self.sort_realm_pc["rev-coin"][faction][realm] = buildSortedTable(realm_table, PCSortByRevCoin)
            self.sort_realm_pc["time_played"][faction][realm] = buildSortedTable(realm_table, PCSortByTimePlayed)
            self.sort_realm_pc["rev-time_played"][faction][realm] = buildSortedTable(realm_table, PCSortByRevTimePlayed)
            self.sort_realm_pc["ilevel"][faction][realm] = buildSortedTable(realm_table, PCSortByiLevel)
            self.sort_realm_pc["rev-ilevel"][faction][realm] = buildSortedTable(realm_table, PCSortByReviLevel)
        end
    end
...
-- By doing it this way, the temporary tables are dropped only once so
...
do
    ...
    function PCSortByRestedXP(a, b)
        ...
        if a and table_to_sort[a] then
            estimated_rested_xp_a =
                AllPlayed:EstimateRestedXP(
                a,
                realm_for_sort,
    ...
    -- Sort function to sort per money
    ...
    -- Sort function to sort per reverse money
    ...
    -- Sort function to sort per time played
    ...
    -- Sort function to sort per reverse time played
...
-- This function calculate the number of XP to reach a particular level.
...
-- This function calculate the number of XP that you need at a particular level to reach
...
function InitXPToLevelCache(game_version, build_version)
    ...
    if AllPlayed.db.global.cache.XPToNextLevel[build_version] ~= nil then
        for level = 1, AllPlayed.max_pc_level do
            if
                AllPlayed.db.global.cache.XPToNextLevel[build_version][level] ~= nil and
                    AllPlayed.db.global.cache.XPToNextLevel[build_version][level] ~= 0
             then
...
function XPToNextLevel( level )
    if XPToNextLevelCache[level] == nil then
        ...
        if level < 32 then
            ...
            if level == 29 then
                XPToNextLevelCache[level] = XPToNextLevelCache[level] + 1 * MXP(level)
            elseif level == 30 then
                XPToNextLevelCache[level] = XPToNextLevelCache[level] + 3 * MXP(level)
            elseif level == 31 then
                XPToNextLevelCache[level] = XPToNextLevelCache[level] + 6 * MXP(level)
            end
...
-- ##
-- ## Note: This function was taken nearly verbatim from the Babble-Class library.
-- ##       I figured seven lines of code was not worth including a library.
-- ##
-- ## Parameter: 	class				= Class name not localized
-- ##
